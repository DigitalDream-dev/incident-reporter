...
export function createCreateRecordAgent(llm: BaseChatModel, tools: DynamicStructuredTool[]) {
...
export function createCreateRecordNode(
  createRecordAgent: ReturnType<typeof createCreateRecordAgent>,
): (state: IncidentWorkflowState) => Promise<{ createRecordResult: string }> {
  return async (state) => {
    ...
    const out = await createRecordAgent.invoke({
      messages: [
        new HumanMessage(
          `Orchestrator approved. Create the Notion database record using this template:\n${JSON.stringify(t, null, 2)}`,
        ),
      ],
    });
